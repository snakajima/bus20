...
\item \textbf{Claude} (\texttt{claude-opus-5}), \textbf{GPT} (\texttt{gpt-5.6-sol}) and \textbf{Gemini} (\texttt{gemini-3.8-flash}), each at its lowest reasoning effort, with structured output constrained to the offered option labels, no tools, no memory across decisions, and no server-side fallback. Claude opus-5 and GPT-5.6 Sol are their vendors' frontier-tier models; Gemini 3.8 Flash is a Flash-class model (the newest stable Gemini at pinning time, the only Pro tier being a preview id), which makes its parity with the other two under matched conditions the more notable. \todo{consider a gemini-3.1-pro-preview run on a subset}
\end{itemize}
